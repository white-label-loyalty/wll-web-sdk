...
export class WllWebSdk {
  ...
  public async init(
    ...
    if (sessionId && sessionUserToken) {
      ...
      if (document.readyState !== 'loading') {
        if ('development' === process.env.NODE_ENV) {
          console.log(
            'Init sdk, document is already ready, just execute code here'
          );
        }
        try {
  ...
  public async signupUsingEmail(emailAddress: string, callback: any) {
    ...
    if (
      ...
      callback(
        new Error("SDK hasn't been initialized yet. Call init() first!")
      );
      return;
    }
    ...
    if (
      ...
      this.userToken.profile.emailAddress &&
      this.userToken.profile.emailAddress !== emailAddress
    ) {
        ...
        userProfile = await new Promise(async (resolve, reject) => {
          ...
          try {
            let response = await fetch(
              baseUrl + 'user_sessions/' + this.sessionId + '/profile',
              {
                method: 'POST',
                headers: {
   ...
   * @param callback
   */
  public async setProfileAsRestricted(
    ...
    if (!this.userToken.profile || !this.userToken.profile.emailAddress) {
      callback(
        new Error(
          "The user hasn't been signedUp yet! Sign up using email first"
        )
      );
      return;
    }
  ...
  public async fillUserDetails(userProfile: UserProfile, callback: any) {
    ...
    if (
      this.userToken.profile &&
      this.userToken.profile.emailAddress &&
      this.userToken.profile.emailAddress !== userProfile.emailAddress
    ) {
  ...
  private async getHashTokenAndFingerprint() {
    ...
    if (!x.hash) {
      ...
      try {
        ...
        if ('development' === process.env.NODE_ENV) {
          console.error(
            'Error while getting fingerprint: ' + JSON.stringify(err)
          );
        }
      }
      ...
      try {
        ...
        if ('development' === process.env.NODE_ENV) {
          console.error(
            'Error while getting location by IP: ' + JSON.stringify(err)
          );
        }
      }
      try {
        const sessionData: any = await this.createUserSession(
          x.hash,
          this.campaignId!,
    ...
    return new Promise(async (resolve, reject) => {
      ...
      setTimeout(async function() {
        Fingerprint2.get(async function(components: any) {
          ...
          var canvas = components.find(
            (component: any) => component.key === 'canvas'
          );
          ...
          var fonts = components.find(
            (component: any) => component.key === 'fonts'
          );
          ...
          var murmur = Fingerprint2.x64hash128(
            canvasValuesString + fontsValueString,
            31
          );
          if ('development' === process.env.NODE_ENV) {
